...
var renderGraphs = function() {
  ...
  return $.ajax({
    ...
    var row = 0;
    var rowType = 'row';
    if ((myFluidFullScreen === true || myFluidFullScreen === 'true') && myFullScreenEnabled === true) {
      rowType = 'row-fluid';
      $('div.graphs div.row').addClass('row-fluid').removeClass('row');
    } 
    $('div.graphs').append('<div class="' + rowType + '"></div>');
    for (var i in d.graphs) {
      ...
      var spanSize = Math.floor( 12 / myColumns );
      if (($('div.graphs div.' + rowType).length - 1) !== row) {
        $('div.graphs').append('<div class="' + rowType + '"></div>');
      }
      $($('div.graphs div.' + rowType)[row]).append('<span id="' + d.graphs[i].uuid + '" class="graph sortable span' + spanSize + '"></div>');
      var myGraphWidth = $($('div.row span.graph')[0]).width();
      if (myFluidFullScreen === true && myFullScreenEnabled === true) {
        myGraphWidth = $($('div.row-fluid span.graph')[0]).width();
      }
      var graphCloseIcon = '<img class="close hidden" src="/img/close.png" />';
      $('div.graphs div.' + rowType + ' span#' + d.graphs[i].uuid).append('<label for="' + d.graphs[i].uuid + '">' + d.graphs[i].name + '</label>');
      $('div.graphs div.' + rowType + ' span#' + d.graphs[i].uuid).append('<a href="/graphs/' + d.graphs[i].uuid + '"><img src="' + constructGraphUrl(c) + '" alt="' + d.graphs[i].name + '" name="' + d.graphs[i].uuid + '" /></a>');
      $('div.graphs div.' + rowType + ' span.graph img').load(function() {
