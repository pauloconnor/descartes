...
var clearGraphs = function() {
  ...
  var rowType = 'row';
  if (myFluidFullScreen === true) {
    rowType = 'row-fluid';
  }
  $('div.graphs div.' + rowType).remove();
  $('.metrics div').remove();
  myPageIndex = 1;
  myRowIndex = 0;
}
...
var selectActiveColumnButton = function() {
  var index = $($('.columns.btn-group button.columns.btn:contains(' + myColumns + ')')).attr('name');
  $('.columns.btn-group button.columns.btn.active').removeClass('active');
  $($('.columns.btn-group button.columns.btn')[index]).addClass('active');
}
...
var enableAutoRefresh = function() {
  ...
  $('.tools.btn-group button.tools.btn.refresh').children('i').addClass('glyphicon-white');
...
var disableAutoRefresh = function() {
  ...
  $('.tools.btn-group button.tools.btn.refresh').children('i').removeClass('glyphicon-white');
...
var setParamsOnLoad = function() {
  ...
  if ($.url().param('nightmode') != undefined) {
    nightmode = $.url().param('nightmode');
  ...
  if ($.url().param('fluid') != undefined) {
    if ($.url().param('fluid') === 'true') {
      myFluidFullScreen = true;
...
var updateUrlParams = function() {
  ...
  myUrl.addSearch({ nightmode: nightmode });
  if (myFullScreenEnabled === true) {
    myUrl.addSearch({ fullscreen: 'true' });
    if (myFluidFullScreen === true){
      myUrl.addSearch({ fluid: 'true' });
    }
  }
...
// If you resize, you'll need to redraw graphs
$(window).resize(function () {
    waitForFinalEvent(function(){
      clearGraphs();
      renderGraphs();
  }, 500, "WindowsResizeRedrawGraphs");
});
...
function enableNightMode() {
  $('body').addClass('night');
  $('div.well').addClass('night');
  $('.reset-search.glyphicon-remove-circle').addClass('glyphicon-white');
  $('.search-query').addClass('input-inverse');
  $('button.tools.btn.nightmode i').removeClass('glyphicon-eye-close').addClass('glyphicon-eye-open');
  $('.collfieldset.collapse.inapse').addClass('night');
  /*$('.btn').addClass('night');*/
  $('.btn').addClass('btn-inverse');
  $('.btn i').addClass('glyphicon-white');
  myGraphTemplate = 'classic';
  myGraphOptions = 'noCache=true&hideLegend=true&template=' + myGraphTemplate;
  }

// deactivate night mode
function disableNightMode() {
  $('body').removeClass('night');
  $('div.well').removeClass('night');
  $('.reset-search.glyphicon-remove-circle').removeClass('glyphicon-white');
  $('.search-query').removeClass('input-inverse');
  $('button.tools.btn.nightmode i').addClass('glyphicon-eye-close').removeClass('glyphicon-white glyphicon-eye-open')
  $('.fieldset.collapse.in').removeClass('night');
  /*$('.btn').removeClass('night');*/
  $('.btn').removeClass('btn-inverse');
  $('.btn i').removeClass('glyphicon-white');
  myGraphTemplate = graphTemplate || 'plain';
  myGraphOptions = 'noCache=true&hideLegend=true&template=' + myGraphTemplate;
}

// activate night mode by click
$('.tools.btn-group button.tools.btn.nightmode').click(function() {
...
$('i.reset-search.glyphicon-remove-circle').hover(function() {
  $(this).addClass('hover');
}, function() {
...
// Clear search form
$('i.reset-search.glyphicon-remove-circle').click(function() {
...
$('.tools.btn-group a.tools.btn.trash').on('click', function() {
  if ($('fieldset.in.collapse').length === 0) {
    ...
    $(this).children('i').removeClass('glyphicon-white');
    clearGraphs();
    renderGraphs();
  }
});
