  ...
  return $.ajax({
    ...
    if (d.length === 0) {
      ...
      for (var i in d) {
        ...
        if (myFluidFullScreen === true || myFluidFullScreen === 'true') {
          rowType = 'row-fluid';
          $('div.graphs div.row').addClass('row-fluid').removeClass('row');
        } 
        $($('div.graphs div.' + rowType)[row]).append('<span id="' + d[i].uuid + '" class="graph span' + spanSize + '"></div>');
        $('div.graphs div.' + rowType + ' span#' + d[i].uuid).append('<label for="' + d[i].uuid + '">' + d[i].name + '</label>');
        $('div.graphs div.' + rowType + ' span#' + d[i].uuid).append('<a href="/graphs/' + d[i].uuid + '"><img src="' + constructGraphUrl(c) + '" alt="' + d[i].name + '" name="' + d[i].uuid + '" /></a>');
